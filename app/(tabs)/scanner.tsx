...
import { addWishlistItem } from "@/db/queries";
...
import { initializeOCRWorker, terminateOCRWorker } from "../../src/utils/ocrWorker";
import { recognizeText } from "expo-vision-ocr";
import { determineDestination, calculateLgsCredit, calculateRetailValue } from "../../src/utils/triage";
...
const queueRowStyles = StyleSheet.create({
  ...
  name: { flex: 1, color: "#e0e0f8", fontSize: 13, fontWeight: "600" },
...
// ── Audio feedback (Web Audio API & expo-av) ────────────────────────────────
async function playTone(type: "success" | "review" | "commit") {
  if (Platform.OS !== "web") {
    try {
      // MOCKED: replace with actual `require('../assets/audio/tone.mp3')` when available
...
export default function ScannerScreen() {
  ...
  // Initialize OCR worker (web + Android only — iOS uses Apple Vision framework)
  useEffect(() => {
    let active = true;
    if (Platform.OS !== 'ios') {
      const setup = async () => {
    ...
    return () => {
      ...
      if (Platform.OS !== 'ios') terminateOCRWorker();
    };
  }, []);
  ...
  // AbortController ref for review modal search — cancels stale in-flight requests
        ...
        if (scanMode === "rapid") {
          ...
          if (similarity >= 0.7) {
            ...
            const queued: QueuedCard = {
              ...
              destination: determineDestination(norm.priceUsd, norm.priceUsdFoil, false),
            };
  ...
  // Debounced review search — fires 500 ms after user stops typing
  const debouncedReviewEditText = useDebounce(reviewEditText, 500);
  useEffect(() => {
    if (!reviewModalVisible || !debouncedReviewEditText.trim()) return;
    handleReviewSearch(debouncedReviewEditText);
  }, [debouncedReviewEditText, reviewModalVisible]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleReviewSearch = useCallback(async (text: string) => {
    ...
    reviewAbortRef.current = new AbortController();
    const { signal } = reviewAbortRef.current;
    setReviewSearching(true);
    try {
      const suggestions = await autocompleteCardName(text.trim());
      if (signal.aborted) return;
      const corrected = suggestions[0] ?? text.trim();
      const results = await searchCardByNameInSet(corrected, selectedSet.code);
      if (signal.aborted) return;
      setReviewResults(results.slice(0, 8));
    } catch (err: any) {
      if (!signal.aborted) setReviewResults([]);
    } finally {
  ...
  const captureManual = async (silent = false): Promise<boolean> => {
    ...
    try {
      if (Platform.OS === "web") {
        ...
        if (cameraRef.current) {
          await Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
          // base64 is only needed for Tesseract (web/Android); skip it on iOS to save memory
          const photo = await cameraRef.current.takePictureAsync({ base64: Platform.OS !== 'ios' });
          if (photo?.uri) {
            ...
            // On iOS skip base64 — Vision only needs the file URI
            const cropped = await ImageManipulator.manipulateAsync(
              photo.uri,
              [{ crop: { originX, originY, width: cropW, height: cropH } }],
              { base64: Platform.OS !== 'ios', format: ImageManipulator.SaveFormat.JPEG, compress: 0.9 }
            );
            ...
            if (Platform.OS === 'ios') {
              // ── Apple Vision OCR ──────────────────────────────────────────────
              // ~30-60 ms on modern iPhones; no WASM, no Web Workers, no battery drain
              const result = await recognizeText(cropped.uri);
              text = result.lines.join(" ");
              console.log("--- VISION OCR DEBUG ---");
              console.log("Lines returned:", result.lines);
            } else {
              // ── Tesseract.js fallback (web / Android) ─────────────────────────
              if (!workerRef.current) return false;
              const { data: { text: t } } = await workerRef.current.recognize(`data:image/jpeg;base64,${cropped.base64}`);
              text = t;
    ...
    const queueTotalValue = rapidQueue.reduce((sum, c) => sum + calculateRetailValue(c.priceUsd, c.priceUsdFoil, false, 1), 0);
    const queueLgsCredit = rapidQueue.filter(c => c.destination === "LGS").reduce((sum, c) => sum + calculateLgsCredit(c.priceUsd, c.priceUsdFoil, false, 1), 0);
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.overlay}>
          {/* Top bar */}
          <View style={[styles.topBar, { paddingTop: isWeb ? 10 : insets.top + 10 }]}>
                ...
                <View style={styles.queueHeaderLeft}>
                  <View style={{ flexDirection: 'column' }}>
                    <Text style={styles.queueROI}>
                      Total Bulk Market: ${queueTotalValue.toFixed(2)}
                    </Text>
                    <Text style={[styles.queueROI, { color: "#22c55e", fontSize: 14, marginTop: 4 }]}>
                      LGS Credit: ${queueLgsCredit.toFixed(2)}
                    </Text>
                  </View>
          ...
          <View style={[styles.bottomBar, { paddingBottom: isWeb ? 36 : insets.bottom + 12 }]}>
        ...
        >
          <View style={styles.pickerContainer}>
            <View style={[styles.pickerHeader, { paddingTop: isWeb ? 60 : insets.top + 16 }]}>
  ...
  const styles = StyleSheet.create({
    ...
    cardFrame: { width: CARD_WIDTH, borderWidth: 2, borderColor: "rgba(255,255,0.7)", borderRadius: 8, justifyContent: "center" },
    targetBoxTop: { width: "100%", height: "100%", paddingHorizontal: 12, justifyContent: "center" },
    targetLabel: { color: "rgba(255,255,0.8)", fontSize: 10, fontWeight: "900", textTransform: "uppercase", letterSpacing: 1 },
    ...
    pickerItemActive: { backgroundColor: "rgba(200,155,60,0.10)" },
    ...
    resultThumb: { width: 54, height: 75, borderRadius: 4, backgroundColor: "#12121a" },
    ...
    reviewNeededItem: {
      flexDirection: "row", alignItems: "center", paddingVertical: 8, borderTopWidth: 1, borderColor: "rgba(245,158,11,0.25)", gap: 10, backgroundColor: "rgba(245,158,11,0.06)"
    },
    reviewNeededText: { color: "#f0f0f8", fontSize: 12, fontWeight: "600", fontStyle: "italic" },
    reviewNeededBadge: { alignSelf: "flex-start", marginTop: 3, backgroundColor: "rgba(245,158,11,0.18)", borderRadius: 6, paddingHorizontal: 7, paddingVertical: 2, borderWidth: 1, borderColor: "#f59e0b" },
    reviewNeededBadgeText: { color: "#f59e0b", fontSize: 10, fontWeight: "800", textTransform: "uppercase", letterSpacing: 0.5 },
